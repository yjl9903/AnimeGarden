...
`apps/web` 是 AnimeGarden 的 TanStack Start 前端应用，外层保留一层 Hono Node server。它负责页面 SSR / hydration、资源浏览和筛选、搜索、收藏夹、详情页、sitemap 输出、OpenAPI 文档页和前端埋点；feed 输出保留在 Node 外壳。
...
| Node server    | `server.mjs` + `node/`                                  | Fly / Node 环境运行 Hono，托管静态资源、保留 `/health`、feed，并把页面请求交给 Start          |
...
4. `/.well-known/mcp/server-card.json` 和 `/llms.txt` 由 TanStack Start route 返回 agent / LLM 发现信息。
5. `/sitemap-index.xml` 和 `/sitemap-*.xml` 由 TanStack Start server route 生成，必要时请求 server/API 后端获取 teams、subjects、detail URL。
...
- 改部署或 feed：先看 `server.mjs`、`node/proxy.ts`、`vite.config.ts`；改 sitemap：先看 `src/sitemap/index.server.ts` 和 `src/routes/sitemap-*.ts`。
