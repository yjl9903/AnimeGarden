...
`src/start.ts` 注册 TanStack Start 全局 request middleware。普通浏览器请求继续走 SSR；`GET` / `HEAD` 且 `Accept: text/markdown` 的已支持页面路径会在进入 HTML SSR 前由 `src/markdown/` 返回应用侧拼装的 Markdown，避免 TanStack Start 的 HTML-only Accept 检查把 agent 请求返回 500。`/llms.txt`、`/openapi.json`、`/.well-known/*` 等 handler route 不进入 Markdown negotiation。
